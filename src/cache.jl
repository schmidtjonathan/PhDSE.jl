...
function FilteringCache(;initial_mean::AbstractVector, initial_covariance::AbstractMatrix)
    FilteringCache(
        IdDict{Tuple{Type, Tuple, AbstractString}, AbstractArray}(
            ...
            (typeof(initial_mean), size(initial_mean), "predicted_mean") => similar(initial_mean),
            (typeof(initial_covariance), size(initial_covariance), "covariance") => copy(initial_covariance),
            (typeof(initial_covariance), size(initial_covariance), "predicted_covariance") => similar(initial_covariance),
        )
    )
end
