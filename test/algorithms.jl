...
@testset "Kalman update vs. Joseph update" begin
    ...
    for y in observations
        standard_m, standard_C = kf_predict(standard_m, standard_C, A(standard_m), Q(standard_m), u(standard_m))
        joseph_m, joseph_C = kf_predict(joseph_m, joseph_C, A(joseph_m), Q(joseph_m), u(joseph_m))

        standard_m, standard_C = kf_correct(standard_m, standard_C, H(standard_m), R(standard_m), y, v(standard_m))
        joseph_m, joseph_C = kf_joseph_correct(joseph_m, joseph_C, H(joseph_m), R(joseph_m), y, v(joseph_m))
    ...
    if PLOT_RESULTS
        ...
        test_plot1 = scatter(1:length(observations), [o[1] for o in observations], color=1)
        test_plot2 = scatter(1:length(observations), [o[2] for o in observations], color=2)
        plot!(test_plot1, 1:length(standard_means), [m[1] for m in standard_means], ribbon=[s[1] for s in standard_stds], label="standard", color=3, lw=3)
        plot!(test_plot2, 1:length(standard_means), [m[2] for m in standard_means], ribbon=[s[2] for s in standard_stds], label="standard", color=3, lw=3)
        plot!(test_plot1, 1:length(joseph_means), [m[1] for m in joseph_means], ribbon=[s[1] for s in joseph_stds], label="joseph", color=4, lw=3)
        plot!(test_plot2, 1:length(joseph_means), [m[2] for m in joseph_means], ribbon=[s[2] for s in joseph_stds], label="joseph", color=4, lw=3)
        test_plot = plot(test_plot1, test_plot2, layout=(1, 2))
...
@testset "Kalman filter (OOP) vs. Sqrt-KF (IIP + OOP)" begin
    ...
    for y in observations
        ...
        iip_sqrt_kf_m, iip_sqrt_kf_C = sqrt_kf_predict!(cache, A(iip_sqrt_kf_m), cholesky(Q(iip_sqrt_kf_m)).U, u(iip_sqrt_kf_m))
        oop_sqrt_kf_m, oop_sqrt_kf_C = sqrt_kf_predict(oop_sqrt_kf_m, oop_sqrt_kf_C, A(oop_sqrt_kf_m), cholesky(Q(oop_sqrt_kf_m)).U, u(oop_sqrt_kf_m))
        ...
        iip_sqrt_kf_m, iip_sqrt_kf_C = sqrt_kf_correct!(cache, H(iip_sqrt_kf_m), cholesky(R(iip_sqrt_kf_m)).U, y, v(iip_sqrt_kf_m))
        oop_sqrt_kf_m, oop_sqrt_kf_C = sqrt_kf_correct(oop_sqrt_kf_m, oop_sqrt_kf_C, H(oop_sqrt_kf_m), cholesky(R(oop_sqrt_kf_m)).U, y, v(oop_sqrt_kf_m))
        # @test kf_m ≈ iip_sqrt_kf_m ≈ oop_sqrt_kf_m
        # @test kf_C ≈ upper_sqrt_to_mat(iip_sqrt_kf_C) ≈ upper_sqrt_to_mat(oop_sqrt_kf_C)
    ...
    @test all([m1 ≈ m2 ≈ m3 for ((m1, C1), (m2, C2), (m3, C3)) in zip(kf_traj, iip_traj, oop_traj)])
    @test all([C1 ≈ C2 ≈ C3 for ((m1, C1), (m2, C2), (m3, C3)) in zip(kf_traj, iip_traj, oop_traj)])
    ...
    if PLOT_RESULTS
        ...
        plot!(test_plot1, 1:length(oop_means), [m[1] for m in oop_means], ribbon=[s[1] for s in oop_stds], label="oop", color=4, lw=3)
        plot!(test_plot2, 1:length(oop_means), [m[2] for m in oop_means], ribbon=[s[2] for s in oop_stds], label="oop", color=4, lw=3)
        plot!(test_plot1, 1:length(kf_means), [m[1] for m in kf_means], ribbon=[s[1] for s in kf_stds], label="kf", color=5, lw=3)
        plot!(test_plot2, 1:length(kf_means), [m[2] for m in kf_means], ribbon=[s[2] for s in kf_stds], label="kf", color=5, lw=3)
        test_plot = plot(test_plot1, test_plot2, layout=(1, 2))
